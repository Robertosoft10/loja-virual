...
$sql = "SELECT * FROM tb_itens WHERE sexo = 'M'  LIMIT $ini, $quant_page_res";
...
<body>
            ...
            <div class="container">
                <div class="nav-depart">
                    <div class="depart-btn">
                        ...
                        <?php 
                        $sql = "SELECT * FROM tb_itens  T_IT JOIN tb_categorias T_CT ON T_IT.categ_item = T_CT.idCateg WHERE sexo = 'M'";
                        $consult_sub = mysqli_query($conexao, $sql);
                        while($rows_sub = mysqli_fetch_array($consult_sub)){
                        ?>
                        <li><a href="itens_masc_categ_list.php?categ_item=<?= $rows_sub['categ_item'];?>"><?= $rows_sub['categoria'];?></a></li>
                        <?php } ?>
                        </ul>
                    </div>
                </div>
    ...
    <section class="product-shop spad">
        ...
        <div class="col-lg-12 text-center">
                    <?php
                    $sql = "SELECT COUNT(idItem) AS num_result FROM tb_itens WHERE sexo = 'M'";
                    ...
                    for($page_previous = $page - $max_links; $page_previous <= $page - 1; $page_previous++){
                        if($page_previous >= 1){
                            echo "<li class='page-item'><a class='page-link' href='itens_masc_list.php?page=$page_previous'>$page_previous</a></li>";
                        }
                    }
                    ...
                    for($page_next = $page + 1; $page_next <= $page + $max_links; $page_next++){
                        ...
                        echo "<li class='page-item'><a class='page-link' href='itens_masc_list.php?page=$page_next'>$page_next</a></li>";
                        }
                    }
                echo "<li class='page-item'><a class='page-link btn primary-btn' href='itens_masc_list.php?page=$quant_page' aria-label='Próximo'>
